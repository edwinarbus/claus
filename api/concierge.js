...
const AGENT_SYSTEM = [
  ...
  '4b) Then propose exactly 6 OPTIONAL follow-up PROMPTS the traveler can tap to send straight to Claus, the in-app chat agent. Write each one\'s TITLE as if the TRAVELER typed it to Claus — a QUESTION or a DIRECTIVE addressed to Claus, in first person ("us/our/we/me") or a plain imperative Claus can act on ("Find…", "Book…", "Swap…", "Move…", "Plan…", "Compare…", "What…", "Which…", "Where…", "Should we…", "How…"). KEEP EACH TITLE SHORT: at most 42 characters (roughly 6-7 words) — it renders on a small two-line tappable chip, so anything longer gets cut off. Cut ruthlessly to the core ask and move any extra context into "detail" instead (e.g. "Swap tomorrow\'s bike ride?" not "Should we swap tomorrow\'s morning bike ride for a gentle walk instead?"). At least TWO of the six must be genuine questions. NEVER a to-do or nudge aimed at the traveler, and never a personal chore Claus can\'t do — no "you/your", never "Lock in your…", "Reserve your…", "Remember to…", "Pack…", or "Note … when you plan". Each must be something CLAUS can actually DO or ANSWER — add/swap/move/remove an itinerary item, find or recommend specific options, or answer a specific planning question — never a vague topic; put the one-line why in "detail" and the day/slot in date/slotKey. EXACTLY HALF (3) are direct follow-ups to THIS brief and tomorrow\'s city (e.g. "Swap the morning bike ride?", "Best Copenhagen tasting menu?"). The OTHER HALF (3) are WHOLE-TRIP prompts NOT tied to tomorrow — spanning the rest of the itinerary: an unbooked hotel or leg, overall pacing, a standout in a later city, a day trip worth adding, budget, must-dos. Good whole-trip prompts: "What\'s still unbooked?" / "Which city are we shortchanging?" / "Day trip from Stockholm?" / "Where should we splurge on dinner?" / "Is our pace too packed?". Do NOT edit the plan yourself; these are prompts the travelers send.',
